...
int spawnRoot(NSString* path, NSArray* args, NSString** stdOut, NSString** stdErr)
{
	...
	do
	{
		if (waitpid(task_pid, &status, 0) != -1) {
            ...
            RLog(@"Child status %d", WEXITSTATUS(status));
		} else
		{
			perror("waitpid");
			_isRunning = NO;
...
NSArray* trollStoreInstalledAppContainerPaths()
{
	...
	if(error)
	{
        ...
        RLog(@"error getting app bundles paths %@", error);
	}
...
SecStaticCodeRef getStaticCodeRef(NSString *binaryPath)
{
	...
	if(binaryURL == NULL)
	{
        ...
        RLog(@"[getStaticCodeRef] failed to get URL to binary %@", binaryPath);
		return NULL;
	}
	...
	if(result != errSecSuccess)
	{
        ...
        RLog(@"[getStaticCodeRef] failed to create static code for binary %@", binaryPath);
		return NULL;
	}
...
NSDictionary* dumpEntitlements(SecStaticCodeRef codeRef)
{
	if(codeRef == NULL)
	{
        ...
        RLog(@"[dumpEntitlements] attempting to dump entitlements without a StaticCodeRef");
		return nil;
	}
	...
	if(result != errSecSuccess)
	{
        ...
        RLog(@"[dumpEntitlements] failed to copy signing info from static code");
		return nil;
	}
